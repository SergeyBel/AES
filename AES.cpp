...
unsigned char * AES::SubWord(unsigned char b[])
{
	const unsigned char sbox[16][16] = {
	0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
	0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
...
unsigned char * AES::RotWord(unsigned char b[])
{
	int i;
	unsigned char *a = new unsigned char[4];
	for (i = 0; i < 4; i++) 
...
unsigned char * AES::XorWords(unsigned char a[], unsigned char b[])
{
	int i;
...
void AES::KeyExpansion(unsigned char key[], unsigned char w[])
{
	...
	while (i < 4 * Nb * (Nr + 1)) 
	{
		...
		if (i / 4 % Nk == 0)
		{
			temp = XorWords(SubWord(RotWord(temp)), Rcon(i / (Nk * 4)));
			delete[]p;
		}
